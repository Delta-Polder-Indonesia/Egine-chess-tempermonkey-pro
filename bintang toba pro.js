...
// @icon         https://cdn.corenexis.com/files/c/6731255720.png
...
(function () {
    ...
    let State = {
        ...
        analysisTotalCplWhite: 0,
        analysisCplMoveCountWhite: 0,
        analysisAcplWhite: "0.00",
        analysisTotalCplBlack: 0,
        analysisCplMoveCountBlack: 0,
        analysisAcplBlack: "0.00",
        previousEvaluationAnalysis: 0,
        acplInitializedAnalysis: false,
    };
    ...
    function cleanupPanelOnly() {
        cleanupEventListeners();
        if (UI && typeof UI._removeAllVisuals === "function") {
    ...
    function runSelfTest() {
        ...
        try {
            ...
            check("Premove cache", typeof premoveSafetyCache !== "undefined");
        } catch (e) {
    ...
    let TimeManager = {
        ...
        _calculateClockSyncDelay: function () {
            ...
            if (myTimeSec <= quickThreshold) {
                let urgencyFactor = Math.max(0.3, myTimeSec / quickThreshold);
                let adjustedDelay = Math.round(quickDelayMs * urgencyFactor);
                log("[TimeManager] LOW TIME! " + myTimeSec.toFixed(1) + "s, delay: " + adjustedDelay + "ms");
                this.isTimePressure = true;
                return Math.max(100, adjustedDelay);
            }
            ...
            let minTime = State.minDelay * 1000;
            let maxTime = State.maxDelay * 1000;
            if (minTime > maxTime) {
                let tmp = minTime;
                minTime = maxTime;
                maxTime = tmp;
            ...
            log("[TimeManager] Clock sync: " + myTimeSec.toFixed(1) + "s left, move#" + moveNum + ", delay: " + finalDelay + "ms");
        ...
        _detectTimeControl: function () {
            ...
            let remainingTime = clockData.playerTime || 0;
            let moveNum = State.moveNumber || 1;

            let estimatedInitialTime = remainingTime + (moveNum * 5);

            if (estimatedInitialTime <= 180) return "bullet";
            if (estimatedInitialTime <= 600) return "blitz";
            if (estimatedInitialTime <= 1800) return "rapid";
            return "classical";
        },
    ...
    let UI = {
        ...
        updateDiagnosticsDisplay: function () {
            ...
            if (cachesEl) { cachesEl.textContent = "PF:" + report.caches.predictedFenCache + " PS:" + report.caches.premoveSafetyCache + " CCT:" + report.caches.cctCache + " TH:" + report.caches.threatCache; }
    ...
    function resignGame() {
        ...
        if (_resignObserver) { _resignObserver.disconnect(); _resignObserver = null; } clearTimeout(_resignTimeout);
        const resignSelectors = ['button[data-cy="resign-button-with-confirmation"]', 'button[data-cy="resign-button"]', 'button[aria-label="Resign"]', 'button[data-cy="game-controls-resign"]', '.resign-button'];
        let resignButton = resignSelectors.map(sel => document.querySelector(sel)).find(btn => btn);
        if (!resignButton) { resignButton = Array.from(document.querySelectorAll("button")).find(btn => { const text = (btn.getAttribute("aria-label") || btn.textContent || "").trim().toLowerCase(); return text === "resign" || text === "menyerah"; }); }
        if (!resignButton) { console.warn("[AutoResign] Tombol resign tidak ditemukan!"); return; }
        function findResignConfirmButton() {
            let btn = document.querySelector('[data-cy="popover-resign-confirmation"] button[data-cy="confirmation-popover-confirm-button"], [data-cy="confirmation-popover"] button[data-cy="confirmation-popover-confirm-button"], button[data-cy="confirmation-popover-confirm-button"], button[data-cy="confirm-yes"], button[data-cy="confirm-modal-primary-button"].cc-button-danger');
            if (btn) return btn;
            return Array.from(document.querySelectorAll("button")).find(function (el) { if (!el || el.disabled) return false; let dataCy = (el.getAttribute("data-cy") || "").toLowerCase(); let cls = (el.className || "").toLowerCase(); let text = (el.textContent || "").trim().toLowerCase(); return dataCy.includes("confirmation-popover-confirm-button") || (cls.includes("cc-button-danger") && (text === "menyerah" || text === "resign")); });
        }
        function clickButtonRobust(button) {
            if (!button) return;
            try { let rect = button.getBoundingClientRect(); let cx = rect.left + rect.width / 2; let cy = rect.top + rect.height / 2; let opts = { bubbles: true, cancelable: true, clientX: cx, clientY: cy, button: 0, buttons: 1 }; button.dispatchEvent(new PointerEvent("pointerdown", Object.assign({ pointerId: 1, pointerType: "mouse", isPrimary: true }, opts))); button.dispatchEvent(new MouseEvent("mousedown", opts)); button.dispatchEvent(new PointerEvent("pointerup", Object.assign({ pointerId: 1, pointerType: "mouse", isPrimary: true }, opts))); button.dispatchEvent(new MouseEvent("mouseup", opts)); button.dispatchEvent(new MouseEvent("click", opts)); } catch (e) { }
            try { button.click(); } catch (e2) { }
        }
        let cleanupDone = false;
        function cleanupResign() { if (cleanupDone) return; cleanupDone = true; if (_resignObserver) { _resignObserver.disconnect(); _resignObserver = null; } if (_resignTimeout) { clearTimeout(_resignTimeout); _resignTimeout = null; } }
        _resignObserver = new MutationObserver(() => { const confirmButton = findResignConfirmButton(); if (confirmButton) { console.log("[AutoResign] Klik tombol konfirmasi resign"); clickButtonRobust(confirmButton); State.gameEnded = true; cleanupResign(); } });
        ...
        try { resignButton.click(); } catch (e) { cleanupResign(); }
        let immediateConfirm = findResignConfirmButton();
        if (immediateConfirm) { console.log("[AutoResign] Konfirmasi resign langsung ditemukan"); clickButtonRobust(immediateConfirm); State.gameEnded = true; cleanupResign(); return; }
        _resignTimeout = setTimeout(cleanupResign, 5000);
    }
    ...
    let AutoMatch = {
        ...
        _LEXICON_EN: { decline: ["decline", "reject", "no"], newGame: ["new game", "new 10 min", "new 5 min", "new 3 min", "new 1 min"], rematch: ["rematch", "play again"] },
        _LEXICON_ID: { decline: ["tolak", "reject", "tidak", "batal"], newGame: ["game baru", "pertandingan baru", "main baru", "10 mnt baru", "5 mnt baru", "3 mnt baru", "1 mnt baru"], rematch: ["tanding ulang", "main lagi", "rematch"] },
        ...
        try: function () {
            ...
            this.inProgress = true; this.attemptCount++;
            log("[AutoMatch] Attempt", this.attemptCount, "starting...");
            let self = this;
            sleep(10000).then(function () { if (!_allLoopsActive || !State.autoMatch) { self.inProgress = false; return false; } return self._executeMatchLogic(); }).then(function (success) { self.inProgress = false; if (!_allLoopsActive || !State.autoMatch) { self.attemptCount = 0; return; } if (!success && self.attemptCount < self.MAX_ATTEMPTS) { scheduleManagedTimeout(function () { self.try(); }, self.ACTION_DELAY_MS); } else { self.attemptCount = 0; } }).catch(function (e) { err("[AutoMatch] Error:", e); self.inProgress = false; });
        },
        _executeMatchLogic: function () {
            let self = this; let lex = self._resolveLexicon();
            return new Promise(function (resolve) {
                if (self._detectRematchRequest()) {
                    let declineBtn = self._findButton(["[data-cy='rematch-decline-button']", "[data-cy='decline-rematch-button']", "button[data-cy*='decline']", "button[data-cy*='reject']", "button[data-cy*='tolak']"], function (txt) { return (lex.decline || []).some(function (kw) { return txt.includes(kw); }); }, (lex.decline || []).map(function (w) { return w.charAt(0).toUpperCase() + w.slice(1); }));
                    if (declineBtn && self._isButtonClickable(declineBtn)) { self._clickElement(declineBtn, "Decline Rematch"); scheduleManagedTimeout(() => { self.inProgress = false; self.try(); }, 3000); resolve(true); return; }
                }
                let newGameBtn = self._findButton(["[data-cy='game-over-modal-new-game-button']", "[data-cy='new-game-button']", "button[data-cy*='new-game']", "button[data-cy*='new_game']"], function (txt) { let hasNewGame = (lex.newGame || []).some(function (kw) { return txt.includes(kw); }); let isRematch = (lex.rematch || []).some(function (kw) { return txt.includes(kw); }) || txt.includes("lagi"); return hasNewGame && !isRematch; }, (lex.newGame || []).map(function (w) { return w.split(" ").map(function (p) { return p.charAt(0).toUpperCase() + p.slice(1); }).join(" "); }));
                if (newGameBtn && self._isButtonClickable(newGameBtn)) { self._clickElement(newGameBtn, "New Game"); if (typeof handleNewGame === 'function') { handleNewGame(); } resolve(true); return; }
                let fallbackBtn = self._findButton(["button", "a[role='button']", "[role='button']"], function (txt, el) { let dataCy = (el.getAttribute("data-cy") || "").toLowerCase(); if (dataCy.includes("rematch")) return false; return (lex.newGame || []).some(function (kw) { return txt.includes(kw); }); });
                if (fallbackBtn && self._isButtonClickable(fallbackBtn)) { self._clickElement(fallbackBtn, "New Game (Fallback)"); if (typeof handleNewGame === 'function') { handleNewGame(); } resolve(true); return; }
                warn("[AutoMatch] No actionable button found"); resolve(false);
            });
        }
    };
